...
def tanHyperbolicDerivative(x):
    return (1 - math.pow(math.tanh(x), 2))


def checkError(predicted_output, epoch):
    # Capping the max epochs in case it doesn't converge anytime soon
    if epoch >= 100:
        print("\nReturning false // Max. number of epoch reached!")
        return False
    else:
        expected_output = [[0], [1], [1], [0]]
        for i, j in zip(expected_output, predicted_output):
            # We're happy if the error gets to 0.1 - i.e., we have that much of a tolerance
            if abs(i[0] - j[0]) > 0.1:
                return True
        return False
...
# Now, gradient error contributes to 0.8

# This means we make bigger jumps // learning rate
lr = 0.8
...
while checkError(predicted_output, epoch):
    ...
    # The next layer takes the previous layer's output as the input
    ...
    # Derivative of the predicted output
